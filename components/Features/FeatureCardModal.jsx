      ...
      <div className="max-w-screen-2xl w-full p-6">
        <div className={`bg-bg-dark rounded-xl shadow-shadow-primary shadow-light/5 p-8 text-black ${isClosing ? 'animate-slide-down' : 'animate-slide-up'} h-fit max-h-[90vh] overflow-y-auto`}>
          ...
          <div className='flex justify-between items-center space-x-5'>
            <h3 id="feature-modal-title" className='text-2xl text-primary font-medium'>Web Solutions</h3>
            ...
            <div>
              ...
              <div className='flex flex-col gap-2 my-5'>
                {featureItem.benefits.map((benefit, index) => (
                  <div key={index} className='ml-5 flex items-center gap-3 text-light text-base'>
                    <GiNinjaStar className='text-primary' /> {benefit}
                  </div>
                ))}
              </div>
              ...
              <div className='flex flex-col gap-2'>
                ...
                <div className='flex flex-wrap gap-3 mt-3 mb-5'>
                  {featureItem.key_technologies_used.map((technology, index) => (
                    <div key={index} className='px-5 py-2 bg-gradient-primary shadow-shadow-primary text-light rounded-md flex items-center gap-2 text-nowrap'>
                      <PiStarFourFill className='text-primary' />{technology}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className='flex flex-wrap gap-3 mt-3 mb-5'>
                  {featureItem.industry_focus.map((site, index) => (
                    <div key={index} className='px-5 py-2 bg-gradient-primary shadow-shadow-primary text-light rounded-md flex items-center gap-2 text-nowrap'>
                      <PiStarFourBold className='text-primary' />{site}
                    </div>
                  ))}
                </div>
              </div>
